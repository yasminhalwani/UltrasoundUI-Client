...
class ManualBased(QtGui.QMainWindow):
    def __init__(self, screen_width_input, screen_height_input):
        ...
        self.second_gaze_press = False
        self.is_gaze_active_in_fullscale_and_zoom_modes = False
    ...
    def setup_roi_layout(self):
        ...
        self.min_roi_width = self.image_height * 0.08
    ...
    def callback_update_POG(self, info):
        ...
        try:
            ...
            # use this for debugging purposes only (it returns the position of the cursor on screen)
            # xpos = float(info['CX']) * self.screen_width
            # ypos = float(info['CY']) * self.screen_height
            # is_valid = int(info['FPOGV'])
    ...
    def set_second_press(self):
        if not self.system_state == SystemState.PRE_ZOOM_A and not self.system_state == SystemState.PRE_ZOOM_B:
            if not self.second_gaze_press:
                self.second_gaze_press = True
            else:
                self.second_gaze_press = False

    def toggle_gaze_press(self):
        if self.second_gaze_press or self.system_state == SystemState.PRE_ZOOM_A or \
    ...
    def gaze_button_press(self):
        self.set_second_press()
        self.toggle_gaze_press()
    ...
    def callback_cursor_monitor(self):
        ...
        if self.system_state == SystemState.ZOOM_A or self.system_state == SystemState.ZOOM_B or \
            ...
            if self.current_trackball_y != self.prev_trackball_y or \
                ...
                px_x = abs(self.prev_trackball_x - self.current_trackball_x)
                px_y = abs(self.prev_trackball_y - self.current_trackball_y)

                # set cursor position to position of roi after zooming in
                if self.last_action == Actions.ZOOM_IN:
                    pos = self.roi.pos()
                    pos = self.context_image_view_box.mapViewToScene(pos)
                    win32api.SetCursorPos((int(pos.x()), int(pos.y())))
                    self.last_action = Actions.PAN
                ...
                if self.system_state == SystemState.ZOOM_B or self.system_state == SystemState.PRE_ZOOM_B:
                    if self.current_trackball_x > right_edge or \
                       self.current_trackball_x < left_edge or \
                       self.current_trackball_y > lower_edge or \
                       self.current_trackball_y < upper_edge:

                        if self.current_trackball_x > right_edge:
                            transition_px_x = abs(self.prev_trackball_x - self.current_trackball_x)
                            win32api.SetCursorPos((int(left_edge), int(y)))
                            if self.system_state == SystemState.ZOOM_B or self.system_state == SystemState.PRE_ZOOM_B:
                                self.resize_routine(transition_px_x, 0)

                        elif self.current_trackball_x < left_edge:
                            transition_px_x = abs(self.prev_trackball_x - self.current_trackball_x)
                        ...
                        if self.current_trackball_y > lower_edge:
                            transition_px_y = abs(self.prev_trackball_y - self.current_trackball_y)
                            win32api.SetCursorPos((int(x), int(upper_edge)))

                            if self.system_state == SystemState.ZOOM_B or self.system_state == SystemState.PRE_ZOOM_B:
                                self.resize_routine(0, transition_px_y)

                        elif self.current_trackball_y < upper_edge:
                            transition_px_y = abs(self.prev_trackball_y - self.current_trackball_y)
                ...
                if self.system_state == SystemState.ZOOM_A or self.system_state == SystemState.ZOOM_B:
                    ...
                    if self.system_state == SystemState.ZOOM_A:
                        self.translate_roi_cursor_zoom_mode()

                    if self.system_state == SystemState.ZOOM_B:
                        if transition_px_y == 0 and transition_px_x == 0:
                            self.resize_routine(px_x, px_y)

                    self.set_view_to_roi_and_pad()

                elif self.system_state == SystemState.PRE_ZOOM_A:
                    self.translate_roi_cursor_pre_zoom_mode()
    ...
    def resize_routine(self, px_x, px_y):
        if self.current_trackball_x > self.prev_trackball_x:
            direction = Direction.RIGHT
            self.resize_roi(direction, px_x)

        elif self.current_trackball_x < self.prev_trackball_x:
            direction = Direction.LEFT
            self.resize_roi(direction, px_x)

        if self.current_trackball_y > self.prev_trackball_y:
            direction = Direction.UP
            self.resize_roi(direction, px_y)

        elif self.current_trackball_y < self.prev_trackball_y:
    ...
    def translate_roi_cursor_pre_zoom_mode(self):

        flags, hcursor, (x, y) = win32gui.GetCursorInfo()

        p = QtCore.QPoint(x, y)

        p = self.image_view_box.mapSceneToView(p)

        roi_pos = self.roi.pos()

        if p.y() + self.roi.size().y() >= self.image_height:
            new_p = self.image_view_box.mapViewToScene(QtCore.QPoint(roi_pos.x(),
                                                        self.image_height - self.roi.size().y()))
            win32api.SetCursorPos((int(new_p.x()), int(new_p.y())))
            p = self.image_view_box.mapSceneToView(new_p)

        if p.x() + self.roi.size().x() >= self.image_width:
            new_p = self.image_view_box.mapViewToScene(QtCore.QPoint(self.image_width - self.roi.size().x(),
                                                                     roi_pos.y()))
            win32api.SetCursorPos((int(new_p.x()), int(new_p.y())))
            p = self.image_view_box.mapSceneToView(new_p)

        if p.y() <= 0:
            new_p = self.image_view_box.mapViewToScene(QtCore.QPoint(roi_pos.x(), 0))
            win32api.SetCursorPos((int(new_p.x()), int(new_p.y())))
            p = self.image_view_box.mapSceneToView(new_p)

        if p.x() <= 0:
            new_p = self.image_view_box.mapViewToScene(QtCore.QPoint(0, roi_pos.y()))
            win32api.SetCursorPos((int(new_p.x()), int(new_p.y())))
            p = self.image_view_box.mapSceneToView(new_p)

        self.roi.setPos((p.x(), p.y()))
        self.context_roi.setPos((self.roi.pos().x(), self.roi.pos().y()))
        self.remove_max_pan_signal()

    def translate_roi_cursor_zoom_mode(self):

        flags, hcursor, (x, y) = win32gui.GetCursorInfo()

        p = QtCore.QPoint(x, y)

        p = self.context_image_view_box.mapSceneToView(p)

        roi_pos = self.roi.pos()

        if p.y() + self.roi.size().y() >= self.image_height:
            new_p = self.context_image_view_box.mapViewToScene(QtCore.QPoint(roi_pos.x(),
                                                        self.image_height - self.roi.size().y()))
            win32api.SetCursorPos((int(new_p.x()), int(new_p.y())))
            p = self.context_image_view_box.mapSceneToView(new_p)

        if p.x() + self.roi.size().x() >= self.image_width:
            new_p = self.context_image_view_box.mapViewToScene(QtCore.QPoint(self.image_width - self.roi.size().x(),
                                                                     roi_pos.y()))
            win32api.SetCursorPos((int(new_p.x()), int(new_p.y())))
            p = self.context_image_view_box.mapSceneToView(new_p)

        if p.y() <= 0:
            new_p = self.context_image_view_box.mapViewToScene(QtCore.QPoint(roi_pos.x(), 0))
            win32api.SetCursorPos((int(new_p.x()), int(new_p.y())))
            p = self.context_image_view_box.mapSceneToView(new_p)

        if p.x() <= 0:
            new_p = self.context_image_view_box.mapViewToScene(QtCore.QPoint(0, roi_pos.y()))
            win32api.SetCursorPos((int(new_p.x()), int(new_p.y())))
            p = self.context_image_view_box.mapSceneToView(new_p)

        self.context_roi.setPos((p.x(), p.y()))
        self.roi.setPos((self.context_roi.pos().x(), self.context_roi.pos().y()))
        self.remove_max_pan_signal()
    ...
    def resize_roi(self, direction, pixels):

        new_height_up = self.roi.size().y() + pixels
        new_y_up = self.roi.pos().y() - pixels / 2

        new_height_down = self.roi.size().y() - pixels
        new_y_down = self.roi.pos().y() + pixels / 2

        new_width_left = self.roi.size().x() - pixels
        new_x_left = self.roi.pos().x() + pixels / 2

        new_width_right = self.roi.size().x() + pixels
        new_x_right = self.roi.pos().x() - pixels / 2
        ...
        if new_width_right + self.roi.pos().x() >= self.image_width:
            pixels = self.image_width - (self.roi.size().x() + self.roi.pos().x())
            new_width_right = self.roi.size().x() + pixels
            new_x_right = self.roi.pos().x() - pixels / 2

        if new_x_right <= 0:
            pixels = self.roi.pos().x()
            new_width_right = self.roi.size().x() + pixels
            new_x_right = self.roi.pos().x() - pixels / 2

        if new_width_left <= self.min_roi_width:
            pixels = self.roi.size().x() - self.min_roi_width
            new_width_left = self.roi.size().x() - pixels
            new_x_left = self.roi.pos().x() + pixels / 2
        ...
        if direction != Direction.NONE:
            ...
            if self.roi.size().y() + pixels >= self.image_height \
                    and self.roi.size().x() + pixels >= self.image_width:
                self.keypress_Q()
    ...
    def set_system_state(self, state):
        ...
        if state == SystemState.FULLSCALE:
            ...
            if self.is_gaze_active_in_fullscale_and_zoom_modes and not self.gaze_activation_held:
                self.gaze_button_press()
            ...
            if self.gaze_activation_pressed or self.gaze_activation_held:
                self.is_gaze_active_in_fullscale_and_zoom_modes = True
            else:
                self.is_gaze_active_in_fullscale_and_zoom_modes = False

            if self.is_gaze_active_in_fullscale_and_zoom_modes:
                self.gaze_button_press()
